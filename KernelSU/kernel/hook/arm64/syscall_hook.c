...
static int __init ksu_find_ni_syscall_slots(int *out_slots, int max_slots)
{
    ...
    ni_syscall = (unsigned long)ksu_resolve_symbol_for_functable_hook(SYS_NI_SYMBOL);
...
static long __nocfi ksu_syscall_dispatcher_from_regs(const struct pt_regs *regs)
{
...
static long __nocfi ksu_syscall_dispatcher(unsigned long arg1, unsigned long arg2,
