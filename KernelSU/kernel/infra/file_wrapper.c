...
#else
static int ksu_wrapper_clone_file_range(struct file *file_in, loff_t pos_in, struct file *file_out, loff_t pos_out,
                                        u64 len)
{
    struct ksu_file_wrapper *data = file_in->private_data;
    struct file *orig = data->orig;
...
static ssize_t ksu_wrapper_dedupe_file_range(struct file *file_in, u64 pos_in, u64 len, struct file *file_out,
                                             u64 pos_out)
{
    struct ksu_file_wrapper *data = file_out->private_data;
    struct file *orig = data->orig;
    return orig->f_op->dedupe_file_range(file_in, pos_in, len, orig, pos_out);
}
#endif
#endif
